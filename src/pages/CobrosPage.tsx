    ...
    <div>
      <div className="flex items-center justify-between mb-6 gap-3 flex-wrap">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          <select value={mes} onChange={(e) => setMes(e.target.value)} className="bg-[var(--ink)] border border-[var(--ink-line)] px-3 py-2 text-sm outline-none focus:border-[var(--gold)] transition-colors capitalize">
            {meses.map((m) => <option key={m.value} value={m.value}>{m.label}</option>)}
          </select>
          <button onClick={exportarCsv} className="flex items-center gap-2 px-3 py-2 border border-[var(--ink-line)] text-[var(--rock)] hover:text-[var(--chalk)] hover:border-[var(--rock)] text-xs uppercase tracking-wide transition-colors">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-6">
        <button onClick={() => setModalOpen('pago')} className="flex items-center gap-3 border border-[var(--ink-line)] bg-[var(--ink-raised)] hover:border-[var(--gold)] p-4 text-left transition-colors">
          <div className="w-10 h-10 flex items-center justify-center border border-[var(--gold)] shrink-0">
        ...
        <button onClick={() => setModalOpen('ingreso')} className="flex items-center gap-3 border border-[var(--ink-line)] bg-[var(--ink-raised)] hover:border-[var(--good)] p-4 text-left transition-colors">
          <div className="w-10 h-10 flex items-center justify-center border border-[var(--good)] shrink-0">
        ...
        <button onClick={() => setModalOpen('gasto')} className="flex items-center gap-3 border border-[var(--ink-line)] bg-[var(--ink-raised)] hover:border-[var(--crit)] p-4 text-left transition-colors">
          <div className="w-10 h-10 flex items-center justify-center border border-[var(--crit)] shrink-0">
      ...
      {tab === 'mov' && !loading && (
        movimientos.length === 0 ? (
          <p className="p-4 text-sm text-[var(--rock-dim)] border border-[var(--ink-line)]">Sin movimientos en este período.</p>
        ) : (
          <div className="border border-[var(--ink-line)] overflow-x-auto">
...
function ReporteMensual({ reporte }: { reporte: Reporte }) {
  ...
  return (
    <div className="space-y-4">
      ...
      {reporte.neto !== 0 && (
        <div className="border border-[var(--ink-line)] bg-[var(--ink-raised)] p-4 text-center">
...
function SummaryCard({ label, value, color, highlight }: { label: string; value: string; color: string; highlight?: boolean }) {
  return (
    <div className={`border p-4 ${highlight ? 'bg-[var(--ink-raised)]' : ''}`} style={{ borderColor: highlight ? color : 'var(--ink-line)' }}>
