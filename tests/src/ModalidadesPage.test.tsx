...
describe('ModalidadesPage', () => {
  it('toggles activo when the Desactivar action is clicked', async () => {
    vi.spyOn(apiClient, 'apiFetch').mockImplementation((path) => {
      if (path === '/api/admin/modalidades') {
        return Promise.resolve({ modalidades: [{ id: '1', nombre: 'Pase x4', tipo: 'CLASES', conZapas: false, cantTickets: 4, precio: 18000, activo: true, comprasCount: 0 }] });
      }
